...
sys_instruct = "You are a data processing assistant. Your job is to clean and structure OCR results into a tabular format that can be directly converted into a CSV file. DO NOT IGNORE 0"
...
def main(image_path):
    ...
    # Step 4: Save to TXT
    output_txt = main_path+"/output.txt"
    save_to_txt(extracted_text, output_txt)

main_path="raw_data/koran/3. Rekening Koran_604_APBN_102024"
# for dir in os.listdir(main_path):
#     if os.path.isdir(main_path+dir):
#         print(dir)
dir_img=os.listdir(main_path)
for img in sorted(dir_img, key=lambda x:int(x.split('.')[0])):
    if img.endswith(".jpg"):
        gambar_input = main_path+"/"+img
        print(f"membuka direktori: {main_path}")
